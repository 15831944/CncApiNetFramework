...
namespace OosterhofDesign
{
	...
	public:
		...
		{
			char * Temp_INI_FILENAME = StringConversie::SYSTEM_STRIN_toCharpoint(INI_FILENAME);
			_LastCNC_RC = (Cnc_Rc)CncConnectServer(Temp_INI_FILENAME);
			if (_LastCNC_RC != Cnc_Rc::CNC_RC_ALREADY_CONNECTED && _LastCNC_RC != Cnc_Rc::CNC_RC_ALREADY_RUNS)
			{
				AppIsPlugin = false;
			}
			delete[] Temp_INI_FILENAME;
			return _LastCNC_RC;
	...
	public:
		...
		static property String^ MacroFileName {
			String^ get()
			{
				char Temp_File[CNC_MAX_PATH] = {};
				_LastCNC_RC = (Cnc_Rc)CncGetMacroFileName(Temp_File);
				return StringConversie::SDT_STRING_toSYSTEM_STRING(Temp_File);
			}
			void set(String^ Value)
			{
				char Temp_File[CNC_MAX_PATH] = {};
				StringConversie::SYSTEM_STRING_toMaxCharLength(CNC_MAX_PATH,Value, Temp_File);
				_LastCNC_RC = (Cnc_Rc)CncSetMacroFileName(Temp_File);
			}
		}
		static property String^ UserMacroFileName
		{
			...
			{
				char Temp_File[CNC_MAX_PATH] = {};
				_LastCNC_RC = (Cnc_Rc)CncGetUserMacroFileName(Temp_File);
				return StringConversie::SDT_STRING_toSYSTEM_STRING(Temp_File);
			}
			void set(String^ Value)
			{
				char Temp_File[CNC_MAX_PATH] = {};
				StringConversie::SYSTEM_STRING_toMaxCharLength(CNC_MAX_PATH, Value, Temp_File);
				_LastCNC_RC = (Cnc_Rc)CncSetUserMacroFileName(Temp_File);
	...
	public:
		...
		{
			char Temp_actCustomerName[CNC_MAX_CUSTOMER_NAME] = {};
		...
		{
			char Temp_requestCode[RequestCodebytes] = {};
		...
		{
			char * Temp_ActivationKey = StringConversie::SYSTEM_STRIN_toCharpoint(activationKey);
			_LastCNC_RC = (Cnc_Rc)CncActivateOption(Temp_ActivationKey);
			delete[] Temp_ActivationKey;
			return _LastCNC_RC;
		}
	...
	public:
		...
		{
			char * Temp_Text = StringConversie::SYSTEM_STRIN_toCharpoint(text);
			int Return = CncGetCurInterpreterLineText(Temp_Text);
			delete[] Temp_Text;
			return Return;
		}
	...
	public:
		...
		{
			char * Temp_extraLine = StringConversie::SYSTEM_STRIN_toCharpoint(extraLine);
			_LastCNC_RC =(Cnc_Rc)CncSetExtraJobOptions(Temp_extraLine, doRepeat, numberOfRepeats);
			delete[] Temp_extraLine;
			return _LastCNC_RC;
		}
		static void GetExtraJobOptions(String^ % extraLine, int % doRepeat, int % numberOfRepeats)
		{
			char Temp_extraLine[CharCharacterBuffer] = {};
			int Temp_doRepeat = -1;
		...
		{
			char Temp_text[CNC_MAX_INTERPRETER_LINE] = {};
			int  Temp_endOfJob = -1;
	...
	public:
		static Cnc_Rc StartJogContinuous(CncCartDouble^ axes,double velocityFactor, int continuous)
		{
			...
			_LastCNC_RC = (Cnc_Rc)CncStartJog(Temp_Axes,velocityFactor,continuous);
			
			return _LastCNC_RC;
		}
		static Cnc_Rc StartJogContinuous(array<double>^ axes, double velocityFactor, int continuous)
		{
			double  Temp_Axes[CNC_MAX_AXES] = {};
			...
			for (int i = 0; i < CNC_MAX_AXES; i++)
			{
				...
				{
					Temp_Axes[i] = axes[i];
					System::Diagnostics::Debug::WriteLine("Axis:"+i+" ="+ Temp_Axes[i]+" Isset");
				}
				else
				{
					break;
					//Temp_Axes[i] = 0;
					//System::Diagnostics::Debug::WriteLine("Axis:" + i + " =" + Temp_Axes[i]);
				}
			}
			_LastCNC_RC = (Cnc_Rc)CncStartJog(Temp_Axes, velocityFactor, continuous);
	...
	public:
		...
		{
			char * Temp_functionName = StringConversie::SYSTEM_STRIN_toCharpoint(functionName);
			char * Temp_fileName = StringConversie::SYSTEM_STRIN_toCharpoint(fileName);
			char * Temp_msg = StringConversie::SYSTEM_STRIN_toCharpoint(msg);

			CncSendUserMessage(Temp_functionName, Temp_fileName, lineNumber, (CNC_ERROR_CLASS)ec, (CNC_RC)rc, Temp_msg);

			delete[] Temp_functionName;
