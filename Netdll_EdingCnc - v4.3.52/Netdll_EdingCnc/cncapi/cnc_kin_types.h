...
typedef enum CNC_KINEMATICS_TYPE
{
	...
	CNC_KINEMATICS_TYPE_COREXY    	    = 5,  /* CoreXY mechanics                        */
...
typedef int  (__stdcall* CncKinematicsInverseMaxFeedType) (  CNC_CART_DOUBLE startPos,
                                                    CNC_CART_DOUBLE endPos, 
                                                    CNC_CART_DOUBLE *maxTrajectVelocity, 
                                                    CNC_CART_DOUBLE *maxTrajectAcceleration, 
                                                    double toolLength);

/* Inverse Kins, calculates max allowable Cartesian velocity and acceleration given start/end pos */
typedef int  (__stdcall* CncKinematicsInverseMaxFeedTypeArc) (  
    CNC_CART_DOUBLE startPos,   //IN: start point of arc
    CNC_CART_DOUBLE endPos,     //IN: end point of arc, including possible ABC move
    CNC_VECTOR center,          //IN: Center point of arc
    CNC_VECTOR normal,          //IN: Unit vector e.g. G17 ==>  X,Y,Z = 0.0, 0.0, 1.0
    int turn,                   //IN: number of full or partial turns CCW in arc
    CNC_CART_DOUBLE *maxTrajectVelocity,        //OUT: Calculated max velocity for each axis by kins DLL
    CNC_CART_DOUBLE *maxTrajectAcceleration,    //OUT: Calculated max velocity for each axis by kins DLL
    double toolLength);         //IN: Length of active tool
