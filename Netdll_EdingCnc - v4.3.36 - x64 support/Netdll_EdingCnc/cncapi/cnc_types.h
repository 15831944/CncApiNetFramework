...
typedef enum CNC_IE_STATE
{
    ...
    CNC_IE_RUNNING_SUB_PAUSED_JOB_STATE, /* single subroutine running from paused state using 2nd interpreter queue */
    CNC_IE_RUNNING_SUB_PAUSED_JOB_WAIT_STATE, /* wait in the state above for MSGDLG */
...
typedef struct _CNC_KIN_CFG
{
    ...
    /* prevent broken tools using a dynamic minimal z machine pos value which the user can define */
    double dynLowZvalue;
    int    dynLowZGuardON;
...
typedef struct _CNC_KIN_STATUS
{
    ...
    int                  m_mcaActive;
    int                  m_tcaActive;
    int                  m_lowZguardActive;
    double               m_lowZposition;
...
typedef struct _CNC_INTERPRETER_CONFIG
{
    ...
    /* dual queue allows user m functions in paused state, 0 => off, 1 => on */
    int  dualQueueEnabled;
...
//Part of JOB_STATUS below
typedef struct _CNC_M3_M9_RUNNING_STATUS
{
...
} CNC_M3_M9_RUNNING_STATUS;

//Part of JOB_STATUS below
typedef struct _CNC_M3_M9_PRESENT_STATUS
{
...
{    
    ...
    /* 1 if running a sub routine in 2nd queue was aborted */
    int aborted2ndQueueRun;
    ...
    /* user reset present in macro.cnc */
    int subResetPresent;

    /* m3 to m9 subroutine status */
    CNC_M3_M9_RUNNING_STATUS m3m9RunningStatus;
    CNC_M3_M9_PRESENT_STATUS m3m9PresentStatus;
...
typedef struct _CNC_PAUSE_STS
{
    ...
    CNC_OFFSET_AND_PLANE pauseOffset;
    int pausePositionValid;
    int pausePositionLine;
    int pauseSpindleIOValue;
    int pauseSpindleDirectionIOValue;
    double pauseSpindleSpeed;
